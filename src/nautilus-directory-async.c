                                             ...
                                             gpointer                   callback_data)
{
    ...
    node = g_hash_table_lookup (directory->details->call_when_ready_hash.active, callback.file);
    ...
    g_hash_table_replace (directory->details->call_when_ready_hash.active, callback.file, node);
                                ...
                                gboolean           active)
{
    ...
    GHashTable *hash = active ? directory->details->call_when_ready_hash.active :
                                directory->details->call_when_ready_hash.inactive;
                      ...
                      gboolean           active)
{
    ...
    callback = link->data;
    link = remove_callback_link_keep_data (directory, link, active);
    g_free (callback);
                          ...
                          ReadyCallback     *callback)
{
    ...
    do
    {
        list = g_hash_table_lookup (directory->details->call_when_ready_hash.inactive, callback->file);
    ...
    do
    {
        list = g_hash_table_lookup (directory->details->call_when_ready_hash.active, callback->file);
        node = g_list_find_custom (list, callback, ready_callback_key_compare);
        if (node != NULL)
        {
...
nautilus_async_destroying_file (NautilusFile *file)
{
    ...
    /* Check for callbacks. */
    node = g_hash_table_lookup (directory->details->call_when_ready_hash.active, file);
    ...
    node = g_hash_table_lookup (directory->details->call_when_ready_hash.inactive, file);
...
call_ready_callbacks_at_idle (gpointer callback_data)
{
    ...
    values = g_hash_table_get_values_as_ptr_array (directory->details->call_when_ready_hash.inactive);
    ...
    for (guint i = 0; i < values->len; i++)
    {
        for (node = values->pdata[i]; node != NULL; node = next)
        {
            ...
            next = remove_callback_link_keep_data (directory, node, FALSE);
...
{
    gboolean found_any;
    GList *active_list, *inactive_list, *node, *next;
    g_autoptr (GPtrArray) values = NULL;

    values = g_hash_table_get_values_as_ptr_array (directory->details->call_when_ready_hash.active);
    ...
    for (guint i = 0; i < values->len; i++)
    {
        ...
        for (node = g_list_last (values->pdata[i]); node != NULL; node = next)
        {
            ...
            {
                active_list = g_list_delete_link (active_list, node);
                g_hash_table_replace (directory->details->call_when_ready_hash.active,
                                      callback->file, active_list);

                inactive_list = g_hash_table_lookup (directory->details->call_when_ready_hash.inactive,
                                                     callback->file);
                inactive_list = g_list_prepend (inactive_list, callback);
                g_hash_table_replace (directory->details->call_when_ready_hash.inactive,
                                      callback->file, inactive_list);
...
nautilus_directory_has_active_request_for_file (NautilusDirectory *directory,
                                                NautilusFile      *file)
{
    if (g_hash_table_lookup (directory->details->call_when_ready_hash.active, NULL) != NULL ||
        g_hash_table_lookup (directory->details->call_when_ready_hash.inactive, file) != NULL ||
        g_hash_table_lookup (directory->details->call_when_ready_hash.active, file) != NULL ||
        g_hash_table_lookup (directory->details->call_when_ready_hash.inactive, NULL) != NULL)
    {
        return TRUE;
    }
          ...
          RequestType   request_type_wanted)
{
    ...
    if (directory->details->call_when_ready_counters[request_type_wanted] > 0)
    {
        ...
        node = g_hash_table_lookup (directory->details->call_when_ready_hash.active, NULL);
